...
Models::PlayerTexture::PlayerTexture(GLuint program, float size, GLuint texture) :
	Player(program, size), mTextureId(texture)
{
	// Bind attributes
...
Models::PlayerTexture::~PlayerTexture()
{
	//glDeleteTextures(1, &mTextureId);
	glDeleteBuffers(1, &mVboTexcoords);
}

void Models::PlayerTexture::draw(const glm::mat4& view) {
	Player::draw(view);
